...
export const data = new SlashCommandBuilder()
  .setName("setbanner")
  .setDescription("Change the bot's profile banner. Upload an image file or provide a direct URL.")
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    await interaction.client.user.setBanner(imageSource);

    const embed = createEmbed({
      title: "Bot Banner Updated",
      description: "The bot's profile banner has been changed successfully.",
      image: imageSource,
      footer: "Bot Config",
      timestamp: true,
    });
    ...
    const isNitroError = err?.message?.toLowerCase().includes("nitro");
    const msg = isNitroError
      ? "Setting a profile banner requires the bot account to have Discord Nitro."
      : err?.code === 50035
        ? "The image could not be processed. Ensure the file is a valid PNG, JPG, GIF, or WebP and does not exceed 8 MB."
        : "The banner could not be updated. This may be due to Discord rate limiting. Please try again in a few minutes.";
